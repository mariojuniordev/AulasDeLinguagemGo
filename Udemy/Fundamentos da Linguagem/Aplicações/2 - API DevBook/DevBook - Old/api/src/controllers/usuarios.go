...
func BuscarUsuarios(w http.ResponseWriter, r *http.Request) {
	nomeOuNick := strings.ToLower(r.URL.Query().Get("usuário"))
	db, erro := banco.Conectar()
	if erro != nil {
		respostas.Erro(w, http.StatusInternalServerError, erro)
		return
	}
	defer db.Close() // Fecha a conexão c o banco
...
func BuscarUsuario(w http.ResponseWriter, r *http.Request) {
	parametros := mux.Vars(r) //returns the path params as a map
...
func AtualizarUsuario(w http.ResponseWriter, r *http.Request) {
	...
	if erro != nil {
		respostas.Erro(w, http.StatusUnprocessableEntity, errors.New("não é possível atualizar um usuário que não seja o seu"))
		return
	}
...
// DeletarUsuario deleta as informações um usuário no banco de dados
